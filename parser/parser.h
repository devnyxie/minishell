...
void				handle_cmd(t_shell_input *shell_input, char **envp,
						t_shell *shell);
void				append_to_linked_list(t_shell_input *shell_input,
						t_cmd *cmd);
// skip_space.c
...
char				*grab_word_with_env(char **input, char **envp,
						t_shell *shell);
char				*grab_unquoted_word(char **input, char **envp,
						t_shell *shell);
// token_utils.c
char				*get_next_token_part(char **input, char **envp,
						t_shell *shell);
...
// parser_cmd_utils.c
int					check_and_handle_variable_assignment(char *cmd_name,
						t_shell *shell);
...
void				set_redirect_fd_on_cmd(t_cmd *cmd, int fd,
						t_redirect_type type);
